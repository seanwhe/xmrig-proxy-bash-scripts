...
function my_config_json () {
	_CONFIG_JSON=$(jo -p access-log-file=$_ACCESS_LOG_FILE access-password=$_ACCESS_PASSWORD algo-ext=$_ALGO_EXT api=$(jo id=$_API_ID worker-id=$_API_WORKER_ID) http=$(jo enabled=$_HTTP_ENABLED host=$_HTTP_HOST port=$_HTTP_PORT access-token=$_HTTP_ACCESS_TOKEN restricted=$_HTTP_RESTRICTED) background=$_BACKGROUND bind=$(jo -a $(jo host=$_BIND_HOST port=$_BIND_PORT tls=$_BIND_TLS)) colors=$_COLORS custom-diff=$_CUSTOM_DIFF donate-level=$_DONATE_LEVEL log-file=$_LOG_FILE mode=$_MODE pools=$(jo -a $(jo algo=$_POOLS_ALGO url=$_POOLS_URL user=$_POOLS_USER pass=$_POOLS_PASS rig-id=$_POOLS_RIG_ID keepalive=$_POOLS_KEEPALIVE enabled=$_POOLS_ENABLED tls=$_POOLS_TLS tls-fingerprint=$_POOLS_TLS_FINGERPRINT daemon=$_POOLS_DAEMON)) daemon-poll-interval=$_POOLS_DAEMON_POLL_INTERVAL retries=$_RETRIES retry-pause=$_RETRIES_PAUSE reuse-timeout=$_REUSE_TIMEOUT tls=$(jo protocols=$_TLS_PROTOCOLS cert=$_TLS_CERT cert_key=$_TLS_CERT_KEY ciphers=$_TLS_CIPHERS ciphersuites=$_TLS_CIPHERSUITES dhparam=$_TLS_DHPARAM ) user-agent=$_USER_AGENT syslog=$_SYSLOG verbose=$_VERBOSE watch=$_WATCH workers=$_WORKERS)
}
